...
exports.videoIndexClean = function (tableService, fileList, processRequest, processResponse, path, response, domain, settings, useCloudData, configName, local) {
	var query = azure.TableQuery.select("RowKey,file").from("movies");
	tableService.queryEntities(query, function(error, entities) {
		if (error) {
			console.log("Query Table: " + error);
		} else {
			console.log("Query Table Entities: " + entities.length);
			for (var e = 0; e < entities.length; e++) {
				var rowKey = entities[e].RowKey;
				var foundIT = false;
				for (var x = 0; x < fileList.length; x++) { if (fileList[x] == entities[e].file) { foundIT = true; } }
				if (!foundIT) {
					console.log("DELETE: " + rowKey + " " + entities[e].file);
					processResponse.write("DELETE: " + rowKey + " " + entities[e].file + ".<br/>\n");
					tableService.deleteEntity("movies", {PartitionKey: "movie", RowKey: entities[e].RowKey}, function (error) { if (error) { console.log("DELETE ERROR: " + error); } } );
				}
			}
		}
	});
